...
## 10. Triển khai hiện tại

Repository dùng multi-agent deterministic Python, không gọi LLM và không cần API key. Mỗi agent nằm trong module riêng dưới `agents/`; typed handoff contracts nằm trong `core/models.py`; repository read-only nằm trong `data_access/`; hard-gate và CLI nằm trong `agents/verifier_agent.py` và `scripts/`.

```bash
# Không có dependency runtime ngoài Python 3.10+ standard library
